 ...
 */
export async function generateComponentEmbedding(component, options) {
 ...
 */
export async function generateComponentEmbeddingWithRetry(component, maxRetries = 3, options) {
    let lastError = null;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (attempt < maxRetries) {
                ...
                console.warn(`⚠️  Embedding failed for ${component.name} (attempt ${attempt}/${maxRetries}), ` +
                    `retrying in ${delay}ms...`);
                await new Promise((resolve) => setTimeout(resolve, delay));
            }
        }
    }
    throw new Error(`Failed to generate embedding for ${component.name} after ${maxRetries} attempts: ${lastError?.message}`);
}
/**
