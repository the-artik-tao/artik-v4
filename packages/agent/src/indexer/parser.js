...
const traverse = typeof traverseModule === "function"
    ? traverseModule
    : traverseModule.default; // eslint-disable-line @typescript-eslint/no-explicit-any
/**
        ...
        traverse(ast, {
            ...
            FunctionDeclaration(path) {
                ...
                if (isReactComponent(path.node, path)) {
                    const component = extractComponentInfo(path.node, path, filePath, imports, exports, false);
                    if (component) {
            ...
            VariableDeclarator(path) {
                ...
                if (t.isIdentifier(path.node.id) &&
                    ...
                    if (isReactComponent(path.node.init, path)) {
                        const component = extractComponentInfo(path.node.init, path, filePath, imports, exports, false, path.node.id.name);
                        if (component) {
                            components.push(component);
                        }
                    }
 ...
 */
function isReactComponent(node, _path // eslint-disable-line @typescript-eslint/no-explicit-any
) {
    ...
    const hasJSX = (n) => {
        ...
        // Check children
        for (const key in n) {
            const value = n[key]; // eslint-disable-line @typescript-eslint/no-explicit-any
 ...
 */
function extractComponentInfo(node, _path, // eslint-disable-line @typescript-eslint/no-explicit-any
filePath, imports, exports, _isDefault, nameOverride) {
 ...
 */
function extractJSXStructure(node) {
    const elements = [];
    const extractElements = (n) => {
        if (t.isJSXElement(n)) {
